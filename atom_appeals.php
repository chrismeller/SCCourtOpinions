<?php
	...
	if ( file_exists( 'cache/atom_appeals.cache' ) ) {
		...
		if ( time() < $mtime + $expiration ) {
			echo file_get_contents( 'cache/atom_appeals.cache' );
			die();
		}
	}
	...
	$title_node = $dom->createTextNode( 'South Carolina Appeals Court Published Opinions' );
	...
	$link->setAttribute( 'href', SCCourtOpinions\AppealsCourt::BASE_URL );
	...
	$author_name = $dom->createElement( 'name', 'Appeals Court of South Carolina' );
	...
	$search_key = 'south carolina appeals court opinions';
	...
	file_put_contents( 'cache/atom_appeals.cache', $xml );
